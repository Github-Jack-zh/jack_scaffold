...
namespace Jack\Scaffold\Http\Controllers;
...
class ScaffoldController extends Controller
{
    ...
    public function index(Content $content)
    {
        ...
        Admin::js(['@fontawesome-iconpicker']);
        Admin::css(['@fontawesome-iconpicker']);
        ...
        //应用名称\路由前缀
        $appName = explode('/',request()->path())[0];
        $permissions = \Dcat\Admin\Models\Permission::selectOptions();
        $menus = Menu::selectOptions();
        return $content
            ->title(trans('admin.scaffold.header'))
            ->description(' ')
            ->body(view(
                'jack.scaffold::helpers.scaffold',
                compact('dbTypes', 'action', 'tables', 'dataTypeMap', 'namespaceBase','appName','permissions','menus')
            ));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if(in_array('route',$creates)){
                $route_model = new JackScaffoldRoute();

                $prefix = explode('/',request()->path())[0];
                if(JackScaffoldRoute::where('prefix',$prefix)->where('route',$route)->exists()){
                    throw new AdminException("Route [$route] already exists!");
                }
                $route_model->prefix = $prefix;
                $route_model->route = $route;
                $route_model->class =  $controller;
                $route_model->save();

            }
            // 创建菜单
            if(in_array('menu',$creates)){
                if(!$menu_name){
                    throw new AdminException('菜单名称不能为空');
                }
                $menu = new Menu();
                $menu->parent_id = $menu_pid;
            ...
            // 创建权限
            if(in_array('permission',$creates)){
                if(!$permission_name){
                    throw new AdminException('权限名称不能为空');
                }
                if(!$permission_slug){
                    throw new AdminException('权限标识不能为空');
                }
                $permission = \Dcat\Admin\Models\Permission::where('slug',$permission_slug)->first();
                $http_path = '/'.trim($route,'/').'*';
                if(optional($permission)->id){
                    if(!str_contains($permission->http_path,$http_path)){
                        $permission->http_path .= ','.$http_path;
                    }
                }else{
                    $permission = new \Dcat\Admin\Models\Permission();
                    $permission->http_path = $http_path;
                }

                $permission->name = $permission_name;
                $permission->slug = $permission_slug;
                $permission->parent_id = $permission_pid;
                $permission->save();
            }
            // Run migrate.
